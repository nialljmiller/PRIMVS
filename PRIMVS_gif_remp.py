...
def gif_genny_phase(primus_line):
    ...
    if len(lightcurve['mjdobs']) > 10 and lightcurve['hfad_mag'].max() - lightcurve['hfad_mag'].min() > 1 and fap < 0.1 and 0.99 < period < 1.01:# and amplitude > 1 and period > 1:
...
def gif_genny_mjd(name):
    ...
    if len(lightcurve['mjdobs']) > 10 and lightcurve['hfad_mag'].max() - lightcurve['hfad_mag'].min() > 3:#and fap < 0.1 and 0.99 < period < 1.01:# and amplitude > 1 and period > 1:
...
primus_fp = '/beegfs/car/njm/OUTPUT/PRIMVS.fits'
...
mask = PRIMVS_hdu[1].data['true_amplitude'] >= 1
