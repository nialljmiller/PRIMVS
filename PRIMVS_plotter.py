...
#python PRIMVS_plotter.py 0 & python PRIMVS_plotter.py 1 & python PRIMVS_plotter.py 2 & python PRIMVS_plotter.py 3 & python PRIMVS_plotter.py 4 & python PRIMVS_plotter.py 5 & python PRIMVS_plotter.py 6 & python PRIMVS_plotter.py 7 & 
...
with fits.open(filename, memmap=True) as hdulist:
    ...
    for row in data:
        ...
        found = any(name_str in filename for filename in files)

        if found:
            print(f'A file with the name "{name_str}" exists in the folder.')
        else:
            ...
            if mag.size > 10:
                ...
                #print(mag, time, period)
                new_fap = TSA_Stats.inference(period, mag, time, knn, model)

                if new_fap < 0.4:
                    base_fp = '/beegfs/car/njm/PRIMVS/LC_FIG/'
                    period_str = str('{:0>8d}'.format(int(period * 100000)))
                    fap_str = str('{:0>3d}'.format(int(new_fap * 100)))

                    output_lc_fp = f'{base_fp}LC_{name_str}_period-{period_str}_fap-{fap_str}.png'
                    ...
                    #lc_plot(mag, magerr, phase, time, period, amplitude, new_fap, output_lc_fp)
